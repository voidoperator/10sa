...
const Days: React.FC<Props> = ({ calendarData, onClickPreviousDays, onClickDay, onClickNextDays }) => {
  ...
  const activeDateData = useCallback(
    (day: number) => {
      ...
      if (dayjs(fullDay).isSame(period.start) && dayjs(fullDay).isSame(period.end)) {
        className = ` ${BG_COLOR['500'][primaryColor]} text-black font-medium rounded-full`;
      } else if (dayjs(fullDay).isSame(period.start)) {
        className = ` ${BG_COLOR['500'][primaryColor]} text-black font-medium ${
  ...
  const hoverClassByDay = useCallback(
    (day: number) => {
      ...
      if (dayHover === fullDay) {
        ...
        className = ` transition-all duration-500 text-black font-medium ${bgColor} ${
  ...
  const buttonClass = useCallback(
    (day: number, type: 'current' | 'next' | 'previous') => {
      ...
      return cn(baseClass, isDateDisabled(day, type) && 'line-through', 'text-gray-500');
    },
