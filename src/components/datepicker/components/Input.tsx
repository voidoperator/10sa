...
const Input: React.FC<Props> = (e: Props) => {
  ...
  const handleInputChange = useCallback(
    ...
    [changeInputText, asSingle, onValueChange, displayFormat, separator, changeDatepickerValue, changeDayHover],
  );
  ...
  const renderToggleIcon = useCallback(
    (isEmpty: boolean) => {
      return typeof toggleIcon === 'undefined' ? <ToggleButton isEmpty={isEmpty} /> : toggleIcon(isEmpty);
    },
    [toggleIcon],
  );
  ...
  const getToggleClassName = useCallback(() => {
    ...
    const defaultToggleClassName =
      'absolute right-0 h-full px-3 text-gray-500 focus:outline-none disabled:opacity-40 disabled:cursor-not-allowed';
